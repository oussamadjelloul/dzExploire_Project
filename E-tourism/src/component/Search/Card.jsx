      ...
      <Link to={"/place/"+detail._id}>
        <div onMouseEnter={handleOnSetView} className='px-4 mb-8'>
          <div className="bg-white border border-gray-200 rounded shadow">
            <a href="#">
                <img className="rounded-t w-full aspect-video" src={detail.images[0]} alt="" />
            </a>
            <div className="p-2">
                <a href="#">
                    <h5 className="mb-2 text-xl leading-7 font-medium text-[#222222]">{detail.place_title.length > 25 ? `${detail.place_title.slice(0, 25)}...` : detail.place_title}</h5>
                </a>
                <p className="mb-2 text-sm leading-5 text-[#5C727D]">{description}</p>
                <div className='flex items-center text-2xl'>
                  <div className='p-2 flex gap-1 items-center'>
                    <MdOutlineWatchLater/>
                    <p className='text-xs leading-4 text-[#5C727D]'>{detail.opening_hour== "Accessible 24 heures sur 24" ? detail.opening_hour : detail.opening_hour+' - '+detail.closing_hour}</p>
                  </div>
                  <div className='p-2 flex gap-1 items-center'>
                    <BiBus/>
